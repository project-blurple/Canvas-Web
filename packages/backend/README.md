...
For local development, install PostgreSQL locally, create a database (for
example, `canvas`), and set `DATABASE_URL` in `.env` to that database.

Before pushing schema changes or seeding, build once so the seed script can
import the built Prisma client:
...
If you want the tests to be automatically rerun when uncommitted test files are
changed, run:
...
We use [Prisma](https://www.prisma.io) for connecting to our database. Running
`pnpm install` also automatically generates type definitions for all the
...
- Run lightweight local-only stack (Jaeger + OpenSearch) while running the
...
- Files:
  ...
  - docker-compose.dev.yml — minimal Jaeger/OpenSearch for host-backed
    development
